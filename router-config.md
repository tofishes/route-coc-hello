...
3. 路由单元：
  ...
  `handle(data, req, res)` 处理接口返回的数据。**需将处理后的数据返回，否则可能得到undefined**。若配置了routerMeta.name，则返回的数据赋值给 res.apiData[routerMeta.name]，否则将直接赋值给 res.apiData，须注意。在数据不受到破坏的情况下，handle(data)中的data，可以拥有三种取值方法：data.getMap(), data.getList(), data.getValue()。取值方法的使用详见[node module value-chain](https://www.npmjs.com/package/value-chain) 。
